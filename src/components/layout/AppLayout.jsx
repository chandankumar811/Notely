...
const AppLayout = () => {
  ...
  const [currentSideBar, setCurrentSideBar] = useState("noteList");
  return (
    <div>
      <div
        className={`flex flex-col w-full h-screen overflow-hidden ${themeClasses.container}`}
      >
        ...
        <div className=" relative w-full h-[calc(100vh-55px)] sm:h-full flex flex-1">
          ...
          {/* Chat area - hidden on mobile when chat is selected */}
          <ChatArea
            currentSideBar={currentSideBar}
            setCurrentSideBar={setCurrentSideBar}
          />
          <div
            className={`absolute w-full h-full ${
              callStatus === "ongoing" || callStatus === "idle"
                ? "pointer-events-none"
                : ""
            }`}
          >
